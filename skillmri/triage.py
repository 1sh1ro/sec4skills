...
def _cross_file_consistency_evidence(ctx: ScanContext, rule_ids: set[str]) -> Evidence | None:
    ...
    split_remote_exec = _disjoint(remote_files, dynamic_files | shell_files)
...
def _kill_chain_evidence(ctx: ScanContext, rule_ids: set[str]) -> list[Evidence]:
    ...
    collection = bool(fine & {"read_secret", "read_agent_state", "read_browser_profile"} or rule_ids & {"secret-file-access", "credential-store-access", "agent-state-access", "browser-profile-access"})
    ...
    remote_supply = bool(fine & {"remote_fetch", "package_install"} or rule_ids & {"remote-code-pipe", "suspicious-package-lifecycle", "doc-remote-bootstrap-required", "update-drift-risk"})
    ...
    if remote_supply and execution:
...
def _files_by_capability(ctx: ScanContext) -> dict[str, set[str]]:
    ...
    for record in ctx.files:
        ...
        if re.search(r"\b(requests\.post|httpx\.post|fetch\(|axios\.post|urlopen|curl\s|wget\s|webhook|collector|https?://)\b", lowered):
            buckets["network"].add(name)
        if re.search(r"\b(curl|wget|git clone|git fetch|git pull|pip install|npm install|npx|https?://)\b", lowered):
            buckets["remote"].add(name)
        if re.search(r"(\.env|id_rsa|\.ssh|\.aws|\.npmrc|git-credentials|login data|cookies|\.claude|\.codex|\.cursor|api[_-]?key|token|secret|password)", lowered):
...
def _remote_exec_or_install_text(ctx: ScanContext) -> bool:
    ...
    return bool(
        re.search(r"\b(curl|wget)\b.{0,140}\|\s*(bash|sh|python|node|ruby|perl)", text, re.I)
        or re.search(r"\b(pip install|npm install|npx|pnpm add|yarn add)\b.{0,120}\b(https?|git\+|latest|main|master|head)\b", text, re.I)
        or re.search(r"\b(requests\.get|httpx\.get|fetch\(|axios\.get|urlopen)\b.{0,200}\b(eval|exec|function\s*\(|import\s*\(|vm\.runin)\b", text, re.I)
    )
